...
class Query(ConnectionDB):
    ...
    def normal_query(self, valor):
        ...
        if valor.isnumeric() and len(valor) <= 2 and 0 < int(valor) <= 49:
            self.cursor.execute(f"""SELECT Placa, Cor, Modelo, Marca, Motorista, Proprietário, Casa
            FROM portaria_bd WHERE Casa LIKE '{valor}' ORDER BY Motorista """)
        else:
            if '[' in valor:
                valor = valor.replace('[', '')

            self.cursor.execute(f"""SELECT Placa, Cor, Modelo, Marca, Motorista, Proprietário, Casa FROM portaria_bd
...
class App(Query):
    ...
    def widgets(self):
        ...
        self.how_button = ttk.Button(master=self.query_entry_frame, text='Como buscar',
                                     style='dark.TButton', command=self.showing_info)
